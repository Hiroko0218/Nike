<template>
  <div>
    ...
    <div style="margin: 0 0 20px 0;">
      ...
      <el-button type="primary" size="small" @click="gotoAddNew()">新增文章</el-button>
    </div>
    ...
    <el-table :data="tableData" border style="width: 100%">
      ...
      <el-table-column label="封面圖" width="100" align="center">
      ...
      <el-table-column prop="authorName" label="作者名字" width="80" align="center"
                       :show-overflow-tooltip="true"></el-table-column>
      <el-table-column prop="categoryName" label="類別名稱" width="100" align="center"
                       :show-overflow-tooltip="true"></el-table-column>
      <el-table-column prop="title" label="標題" width="160" align="center"
                       :show-overflow-tooltip="true"></el-table-column>
      <el-table-column prop="brief" label="簡介" width="165" align="center"
                       :show-overflow-tooltip="true"></el-table-column>
      <el-table-column label="讚數/倒讚數" width="100" align="center">
        <template slot-scope="scope">
          <div>{{ scope.row.upCount }} / {{ scope.row.downCount }}</div>
        </template>
      </el-table-column>
      <el-table-column prop="clickCount" label="瀏覽量" width="80" align="center"></el-table-column>
      <el-table-column prop="commentCount" label="評論量" width="80" align="center"></el-table-column>
      ...
      <el-table-column label="操作" width="161" align="center">
        <template slot-scope="scope">
    ...
    <!-- 顯示文章詳情的對話框 -->
    <el-dialog class="articles-info" :title="articles.title" :visible.sync="infoDialogVisible">
      <el-descriptions :column="1" border label-class-name="spu-info-label">
        ...
        <el-descriptions-item label="作者ID">{{ articles.authorId }}</el-descriptions-item>
        <el-descriptions-item label="作者名字">{{ articles.authorName }}</el-descriptions-item>
        <el-descriptions-item label="類別ID">{{ articles.categoryId }}</el-descriptions-item>
        <el-descriptions-item label="類別名稱">{{ articles.categoryName }}</el-descriptions-item>
        <el-descriptions-item label="標題">{{ articles.title }}</el-descriptions-item>
        <el-descriptions-item label="簡介">{{articles.brief}}</el-descriptions-item>
        <el-descriptions-item label="關鍵詞">{{ articles.keywords }}</el-descriptions-item>
        <el-descriptions-item label="IP地址">{{ articles.ip }}</el-descriptions-item>
        <el-descriptions-item label="排序序號">{{ articles.sort }}</el-descriptions-item>
        <el-descriptions-item label="是否推薦">{{ articles.isRecommend == 0 ? '否' : '是' }}</el-descriptions-item>
        <el-descriptions-item label="是否顯示">{{ articles.isDisplay == 0 ? '否' : '是' }}</el-descriptions-item>
        <el-descriptions-item label="審核狀態">{{ checkStateText[articles.checkState] }}</el-descriptions-item>
        <el-descriptions-item label="讚數量">{{ articles.upCount }}</el-descriptions-item>
        <el-descriptions-item label="倒讚數量">{{ articles.downCount }}</el-descriptions-item>
        <el-descriptions-item label="評論總量">{{ articles.commentCount }}</el-descriptions-item>
        <el-descriptions-item label="瀏覽量">{{ articles.clickCount }}</el-descriptions-item>
        <el-descriptions-item label="封面圖">
          <el-image class="picture-item"
                    :src="articles.coverUrl"
                    fit="fit"></el-image>
        </el-descriptions-item>
      </el-descriptions>
      <div slot="footer" class="dialog-footer">
...
export default {
  ...
  methods:{
    // 跳轉到新增頁面
    gotoAddNew() {
      this.$router.push('/admin/content/article/add-new');
    },
    // 打開審核對話框
    openCheckDialog(articles) {
      this.checkForm.id = articles.id;
      this.checkForm.title = articles.title;
    ...
    // 切換【顯示】狀態
    toggleIsDisplay(articles) {
      let isDisplayStateText = ['不顯示','顯示'];
      let url = BaseUrl.ADMIN_CONTENT + '/articles/' + articles.id;
      if (articles.isDisplay == 1) {
        url += '/display';
      } else {
        url += '/hidden';
      }
      ...
      TokenAxios.post(url).then((response) => {
        ...
        if (jsonResult.state == 20000) {
          this.$message({
            message: isDisplayStateText[articles.isDisplay] + '成功！',
            type: 'success'
          });
        } else if (jsonResult.state == 40400) {
          this.$alert(jsonResult.message, '錯誤', {
    ...
    // 打開編輯對話框
    openEditDialog(articles) {
      let title = '提示';
      let message = '您正在嘗試編輯【' + articles.id + '-' + articles.title + '】文章，抱歉，此功能尚未實現……';
    ...
    // 打開刪除確認框
    openDeleteConfirm(articles) {
      let message = '將永久刪除【' + articles.title + '】此文章，是否繼續？';
      this.$confirm(message, '提示', {
        confirmButtonText: '確定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        this.handleDelete(articles);
      }).catch(() => {
      });
    },
    // 執行刪除
    handleDelete(articles) {
      let url = BaseUrl.ADMIN_CONTENT + '/articles/' + articles.id + '/delete';
      console.log('url = ' + url);
...
.articles-info .spu-info-label {
  width: 120px;
}

.articles-info .picture-item {
  width: 80px;
  height: 80px;
  margin-right: 16px;
  margin-bottom: 10px;
