...
AOP是由AspectJ提供的技術，Spring框架很好的支持了AOP。

AOP主要解決了橫切關注的問題（若幹個不同的方法都需要解決的問題），典型的應用場景有：錯誤的處理、事務管理、安全檢查、日志等等。

在Spring Boot項目中，需要添加`spring-boot-starter-aop`依賴，才可以進行AOP編程。
...
public class TimerAspect {
    ...
    // 連接點（JoinPoint）：程序執行過程中的某個節點，可能是方法的調用，或拋出異常
    // 切入點（Point Cut）：是匹配1個或多個連接點的表達式
    // 通知（Advice）：包含了切入點表達式與執行的方法
    // -------------------------------------------------------------
    // Advice的種類（使用不同Advice時，方法的參數列表將不同）：
    // -- @Before：前置，你的代碼將在連接點【之前】執行
    // -- @After：後置，你的代碼將在連接點【之後】執行
    // -- @AfterReturning：返回之後，你的代碼將在連接點【成功返回之後】執行
    // -- @AfterThrowing：拋出異常之後，你的代碼將在連接點【拋出異常之後】執行
    // -- @Around：環繞，你的代碼可以在連接點【之前和之後】執行
    // -------------------------------------------------------------
    // 各Advice的執行節點大致是：
    // @Around：開始
    // try {
    ...
    // -- 可以使用1個星號作為通配符，用於匹配任意內容，只會且必須匹配1次
    // -- 可以使用2個連接的小數點作為通配符，可以匹配若幹次（0~n次），只能用於包名和參數列表
    //                 ↓ 返回值類型
    //                   ↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓ 包名
    //                                                    ↓ 類型名
    //                                                      ↓ 方法名
    ...
    public Object timer(ProceedingJoinPoint pjp) throws Throwable {
        ...
        // 注意-2：調用連接點時，必須將異常拋出（或者，使用try...catch處理時，在catch代碼塊再次拋出異常）
