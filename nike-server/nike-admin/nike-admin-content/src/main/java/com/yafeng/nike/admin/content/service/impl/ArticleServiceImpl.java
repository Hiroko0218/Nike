...
import com.yafeng.nike.admin.content.dao.persist.repository.*;
import com.yafeng.nike.admin.content.dao.search.IArticleSearchRepository;
...
public class ArticleServiceImpl implements IArticleService {
    ...
    public void addNew(CurrentPrincipal currentPrincipal, String remoteAddr, ArticleAddNewParam articleAddNewParam) {
        ...
        if (category == null) {
            String message = "發布文章失敗，選擇的類別不存在！";
            log.warn(message);
            throw new ServiceException(ServiceCode.ERROR_NOT_FOUND, message);
        }

        if (category.getEnable() != 1) {
            String message = "發布文章失敗，選擇的類別已經被禁用！";
            log.warn(message);
    ...
    public void delete(Long id) {
        log.debug("開始處理【根據ID刪除文章】的業務，參數：{}", id);
        ArticleStandardVO queryResult = articleRepository.getStandardById(id);
        if (queryResult == null) {
            String message = "刪除文章失敗，嘗試刪除的文章數據不存在！";
            log.warn(message);
            throw new ServiceException(ServiceCode.ERROR_NOT_FOUND, message);
        }
        ...
        if (rows != 1) {
            String message = "刪除文章失敗，服務器忙，請稍後再嘗試！";
            log.warn(message);
    ...
    private void updateCheckById(CurrentPrincipal currentPrincipal, Long id, Integer checkState, String remarks) {
        ...
        if (currentArticle == null) {
            String message = "將文章的審核狀態修改為【" + CHECK_STATE_TEXT[checkState] + "】失敗，文章數據不存在！";
            log.warn(message);
            throw new ServiceException(ServiceCode.ERROR_NOT_FOUND, message);
        }
    ...
    private void updateDisplayById(Long id, Integer displayState) {
        ArticleStandardVO currentArticle = articleRepository.getStandardById(id);
        if (currentArticle == null) {
            String message = "將文章的顯示狀態修改為【" + DISPLAY_STATE_TEXT[displayState] + "】失敗，文章數據不存在！";
            log.warn(message);
            throw new ServiceException(ServiceCode.ERROR_NOT_FOUND, message);
        }
